...
KSQLDB_ID=$(echo "$ksqldb_response" | grep ID | grep -o 'lksql[^ ]\+')
...
export KSQLDB_ID=$(echo "$ksqldb_response" | grep ID | grep -o 'lksql[^ ]\+')
export KSQLDB_ENDPOINT=$(echo "$ksqldb_response" | grep Endpoint | grep -o 'https[^ ]\+')
